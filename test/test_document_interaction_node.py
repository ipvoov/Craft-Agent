...
async def test_document_interaction_node() -> bool:
    """测试 document_node 是否能基于交互设计步骤生成 interaction.md 风格的文档。"""
    print("\n" + "=" * 60)
    print("🧪 测试 document_node - 交互设计文档步骤 (interaction.md)")
    print("=" * 60)

    try:
        # 1. 构造包含交互设计步骤的 WebPlan
        print("\n📋 步骤 1: 构造 WebPlan（仅包含交互设计文档步骤）")

        # 使用你在终端中给出的“步骤 1: 创建交互设计文档(interaction.md)”内容
        step1_description = (
            "明确全站交互流程：1) 左侧导航面板与顶栏标签页的联动逻辑 2) 文章目录树的展开/折叠交互 "
            "3) 代码块的复制与高亮机制。文档需包含路由跳转关系图（首页→文章页→标签页→关于页）和组件交互状态说明，输出为interaction.md"
        )

        plan = WebPlan(
            thought="测试交互设计文档步骤",
            title="极客博客站点 Web 生成测试计划",
            steps=[
                WebStep(
                    title="创建交互设计文档(interaction.md)",
                    description=(
                        "步骤 1: 创建交互设计文档(interaction.md)\n"
                        "类型: document\n"
        ...
        print("✅ WebPlan 已构造，包含 1 个 document 步骤，并在描述中显式包含 interaction.md 关键词")
        ...
        print("\n📋 步骤 3: 调用 document_node 执行当前交互设计文档步骤")
        config = {"configurable": {}}
        ...
        if not exec_res:
            ...
            print("\n✅ 当前步骤已生成交互设计文档内容 (execution_res 非空)")

        print(
            "\n📂 请手动检查以下文件是否已生成预期风格的 Markdown 文档：\n"
            "   - source/project_01/interaction.md (如使用了 write_file_tool)\n"
            "   - 或你在会话中另行指定的相对路径\n"
        )

        print("\n🎉 document_node 交互设计文档步骤测试完成")
        return True
...
def main() -> None:
    """命令行入口：运行交互设计文档节点测试。"""
    print("🦌 DeerFlow - document_node 交互设计文档测试")
    print(f"项目根目录: {project_root}")

    success = asyncio.run(test_document_interaction_node())

    if success:
        print("\n✨ 测试结束：document_node 交互设计文档步骤执行成功")
        sys.exit(0)
    else:
